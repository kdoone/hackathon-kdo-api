...
export const friendRequestValidate = [
    check('usernameOrId')
        .trim()
        .exists().withMessage({ statusCode: 1, message: 'usernameOrId is required' })
        .bail()
        .not().isEmpty().withMessage({ statusCode: 2, message: 'usernameOrId is empty' })
        .bail()
        .isLength({ max: 32 }).withMessage({ statusCode: 3, message: 'usernameOrId shall not exceed 32 characters' })
];
...
export const friendRequest = async (req: ReqWithPayload, res: Response, next: NextFunction) => {
    try {
        ...
        const uid = await User.getId('uid', usernameOrId);
        const username = await User.getId('username', usernameOrId);
